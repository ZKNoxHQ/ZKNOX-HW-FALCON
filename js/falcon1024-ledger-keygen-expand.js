#!/usr/bin/env node
// falcon1024-ledger-keygen-expand.js — v0.5.2 integration
//
// Drives the on-device INS_FALCON_KEYGEN_EXPAND (0x34) handler through
// its 6-phase protocol and collects the 122864 B wire blob.
//
...
//
// Wire output (122864 B):
//   0      L0 ct                 8192 B
//   8192   L0 tag                  16 B
//   8208   right subtree records 57328 B  (1023 nodes with ct||tag)
...
const CLA = 0xE0;
const INS_FALCON_KEYGEN_EXPAND = 0x34;
...
const L0_RECORD_SZ = FN * 8 + NODE_MAC_LEN;                // 8208
const SUBTREE_NODES = (1 << (DFS_LOGN + 1)) - 1;            // 1023
const tsz = (l) => (l + 1) * (1 << l);
const SUBTREE_RECORD_SZ =
    tsz(DFS_LOGN) * 8 + SUBTREE_NODES * NODE_MAC_LEN;        // 57328
...
async function keygenExpand(transport) {
    ...
    // Phase 2 — GET_L0 by chunk index (random access within the 8208 B record)
    ...
    // Phase 4 — GET_NEXT_R, sequential pulls until 57328 B collected
    ...
    // Phase 6 — GET_NEXT_L, sequential pulls until another 57328 B collected
...
//   3. Run FALCON_KEYGEN_EXPAND → collect the 122864 B wire blob.
...
if (require.main === module) {
    ...
    (async () => {
        ...
        try {
            ...
            const fs = require('fs');
            const outPath = process.argv[2] || 'falcon1024_wire.bin';
