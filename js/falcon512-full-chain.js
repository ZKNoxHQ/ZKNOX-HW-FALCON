#!/usr/bin/env node
/* falcon512-full-chain.js — Full Falcon-512 v0.1.0 closed-loop test.
 *
 *   1. FALCON_KEYGEN  (BIP-32 derived seed, on-device)
 *   2. FALCON_GET_PK  (collect pk h)
 *   3. FALCON_KEYGEN_EXPAND (collect 57328 B wire)
 ...
 *
 * USAGE
 *    node falcon512-full-chain.js [<msg-utf8-or-hex>] [--mode device|host] [--testvec]
 *
...
const { keygenExpand, TOTAL_WIRE_SZ } = require('./falcon512-ledger-keygen-expand');
const { sign } = require('./falcon512-ledger-sign');
const { verifyRaw, hashToPoint } = require('./falcon512-verify');
...
async function main() {
    ...
    console.log('═══ Falcon-512 v0.1.0 full-chain test ═══');
    ...
    /* References */
    const refWirePath = 'falcon512_yellow12_wire.bin';
    const refPkPath   = 'falcon512_yellow12_pk.bin';
    if (!fs.existsSync(refWirePath) || !fs.existsSync(refPkPath)) {
        ...
        console.error(`  - ${refPkPath} (1024 B)`);
        process.exit(1);
    }
