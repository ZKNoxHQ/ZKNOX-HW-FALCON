...
/*
 * Falcon Round 3 mode (compile with -DFNDSA_FALCON_R3=1): sign with a raw
 * Falcon private key (f | g | F, int8, 3n bytes) over nonce || msg, output
 * header + nonce + raw s2 (int16). See sign.c.
 */
size_t fndsa_falcon_sign_seeded_temp(unsigned logn, const int8_t *fgF,
