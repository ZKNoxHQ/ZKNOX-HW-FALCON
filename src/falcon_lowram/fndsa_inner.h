...
/* If FNDSA_FALCON_R3 is 1, then the signing core produces Falcon Round 3
   signatures instead of FN-DSA ones (same lattice math, same parameters):
     - hash_to_point reads 16-bit words big-endian (Falcon) and hashes
       nonce || message (arbitrary length, no BUFF / mu),
     - the private key is given raw (f | g | F, int8, 3n bytes),
     - the signature output is the raw s2 polynomial (int16, host encodes
       with the Falcon MSB-first compressed codec if needed).
   The FN-DSA API (mu, ctx, hash id, LSB-first codecs) is not affected by
   this option except for the internal hash_to_point endianness. */
#ifndef FNDSA_FALCON_R3
#define FNDSA_FALCON_R3   0
#endif

/* If FNDSA_SAMPLER_PROTECT is 1, then the Gaussian sampler is the
   power-analysis protected variant of Lin et al. (PKC 2025), see
   sign_sampler.c. Only supported with the plain (non-SIMD) code path. */
#ifndef FNDSA_SAMPLER_PROTECT
#define FNDSA_SAMPLER_PROTECT   0
#endif
...
void hash_to_point(unsigned logn,
	const uint8_t *nonce, const uint8_t *mu, size_t mu_len, uint16_t *c);
