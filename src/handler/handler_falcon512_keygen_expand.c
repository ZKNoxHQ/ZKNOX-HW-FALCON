...
static void compute_l0_phase(void) {
     ...
     * in g_zknox (saves 1024 B BSS, shared with Falcon-1024). Use SLOT(0)
     * as the 4 KB tmp buffer for complete_private — gram_phase will
     * overwrite the slots anyway. For Falcon-512, the buffer is half size
     * but only the first 512 entries are used. */
    int8_t G_buf[FN];
