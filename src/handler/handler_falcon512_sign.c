/*
 * handler_falcon512_sign.c — Falcon-1024 sign for Ledger Nano S Plus
 *
...
#define HN        (FN >> 1)        /* 256 */
#define DFS_LOGN  8                /* levels 1-9 = Falcon-512 DFS (FLOGN-1) */
...
} falcon512_sign_ctx_t;

#define sctx (*(falcon512_sign_ctx_t *)(void *)g_zknox._falcon_sign_area)
...
/* ================================================================
 * DFS stack helpers (levels 1-9 = Falcon-512, logn=9, polys ≤256)
 * ================================================================ */
...
/* ================================================================
 * Standard DFS (Falcon-512, mirrors Falcon-1024 with smaller polys)
 * ================================================================ */
...
static void do_compute_s0_sqn(void) {
     ...
     * (saves 1024 B BSS, shared with Falcon-1024). complete_private needs
     * 2*FN*sizeof(uint16_t) = 2 KB of tmp; sctx.ws is exactly HN*8 = 2 KB. */
    int8_t G_local[FN];
...
int handler_falcon512_sign(buffer_t *cdata, uint8_t p1, uint8_t p2) {
    ...
    case 0x00: /* INIT */
        memset(&sctx, 0, sizeof(falcon512_sign_ctx_t));
    ...
    case 0x04: { /* FEED_SWAP
        ...
        if (auth) {
            ...
            const size_t ct_bytes = FN * sizeof(fpr); /* Falcon-512: 4096 */
            const uint8_t *src = cdata->ptr;
            size_t remaining   = cdata->size;
            ...
            if (sctx.l0_phase == L0_WAIT_L0) {
                ...
                do_level0_enter();
                sctx.tree_byte_offset = ct_bytes;  /* FN * 8 — Falcon-512: 4096 */
