 ...
 *
 *   0x50 FALCON_LR_KEYGEN   P1=0. Derives the SLIP-10 seed, and either
 *        returns the stored public key (record bound to that seed) or runs the
 *        fixed-point keygen (~22.6 KB temp for 1024) and persists f, g, F, h.
 ...
 *        0x91 GET_NONCE (40 bytes). No KEYGEN needed in the session: the
 *        seed is re-derived and the key read from NVM.
