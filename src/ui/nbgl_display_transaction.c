...
// Buffer for the hash hex string (32 bytes = 64 hex chars + null)
static char g_hash_str[65];

int ui_display_blind_sign_hash(void) {
    explicit_bzero(g_hash_str, sizeof(g_hash_str));

    if (G_context.req_type != CONFIRM_BLIND_SIGN_HASH || G_context.state != STATE_PARSED) {
        G_context.state = STATE_NONE;
        return io_send_sw(SWO_CONDITIONS_NOT_SATISFIED);
    }

    if (format_hex(G_context.tx_info.m_hash, 32, g_hash_str, sizeof(g_hash_str)) == -1) {
        return io_send_sw(SWO_INCORRECT_DATA);
    }

    // Single pair: the hash
    pairs[0].item = "Hash";
    pairs[0].value = g_hash_str;
